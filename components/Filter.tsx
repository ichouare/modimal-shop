...
function Filter() {
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4">
      ...
      {Object.values(state).filter((item) => item !== null).length ? (
        <div className="w-full h-fit flex flex-col gap-4 ">
          {Object.entries(state)
            .filter(([, item]) => item !== null)
            .map(([key, item]) => (
              <span
                className="block w-[184px] bg-primary-50 h-[40px]  flex items-center justify-between px-4 "
                key={key}
              >
                <p className="bodyLG text-black">{key}</p>
                <Button
                  onClick={() =>
                    dispatch({
          ...
          <div className="grid grid-cols-2 grid-flow-col gap-2">
            <Button
              variant="default"
              className="max-w-full h-[40px] bg-white text-black border border-black"
              onClick={() =>
                dispatch({
                  ...
                  if (item.type === "color")
                    return (
                      <div key={idx} className="flex items-center gap-[15px]">
                        <Checkbox
                          checked={state[item.type] == val.value}
                          onCheckedChange={() => {
                            dispatch({
                              type: actionType.ADDITEM,
                              value: { [item.type]: val.value },
                            });
                          }}
                          className="size-4.5"
                          value={val.value}
