...
function SignUpForm({ setAlreadyHave }: Tprops) {
  const {
    register,
    handleSubmit,
  ...
  return (
    <section className="flex flex-col items-center gap-[30px]">
      ...
      <form
        onSubmit={onSubmit}
        className="h-full flex flex-col items-center  justify-center gap-[10px] w-[400px] bg-white"
      >
        <div className="w-full flex flex-col items-center  gap-[4px]">
          <Input
            className="border-gray-600 w-[392px]"
            type="email"
            {...register("email")}
            placeholder="Enter Email"
          />
          {errors?.email && (
            <p className="bodyXS text-error w-full text-start">
        ...
        <div className="w-full flex items-center justify-around  ">
          <Link href="#" className=" text-primary bodySM  w-full">
        ...
        <Button
          ...
          className="w-full rounded-none h-[40px] cursor-pointer buttonSM disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      <div className="w-full flex flex-col items-center justify-center  gap-[20px] px-10">
        ...
        <MediaAuth />
        <p className=" w-[400px] text-center text-black bodySM">
          New to modimal?{" "}
          <span
            className="text-primary cursor-pointer"
            onClick={() => setAlreadyHave(false)}
          >
            {" "}
            create an account{" "}
          </span>{" "}
      ...
      {/* success Dialog  */}
      <CostumizeDialog
        title="welcome to modimal"
        open={isopen}
        setIsOpen={setIsopen}
      >
        <div className="flex-1 h-full  flex flex-col items-center justify-center gap-[40px]">
          <span className="bodyLG">
            <i>Elegance in simplicity, Earth’s harmony</i>
          </span>
          <h4 className="text-black">
            is it your first experience on modimal?
          </h4>
          <Button variant="default" className="buttonLG">
